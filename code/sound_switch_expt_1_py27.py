...
# Stimuli can be either pure tones or guitar chords, with the
# order of stimulus presentation decided via a different random
...
import random
import socket
from stefan_utils_py27 import tabify
from stefan_utils_py27 import rgb2psychorgb
from stefan_utils_py27 import quit_experiment
from stefan_utils_py27 import get_subject_info
from stefan_utils_py27 import make_data_file
from stefan_utils_py27 import make_subject_file
from stefan_utils_py27 import write_to_file
from stefan_utils_py27 import show_instructions
...
def create_rating_scale():
    ...
    rating_scale.pulseColor = [
        rgb2psychorgb([91, 189, 255]) for i in range(frames_per_cycle)
    ]
...
experiment_clock = core.Clock()  # time since start of first trial
block_clock = core.Clock()  # time since start of first trial of the block
trial_clock = core.Clock()  # time since start of the trial
...
this_song_info = {"switch_rate": "0.5", "exemplar": 0}
this_song_info["song_name"] = f"{stimulus_dir}practice_pure_tones.mp3"
...
instructions_list.append(
    ...
    "For example, you might hear an instrument play one note, "
    "and then play a different note. \n\n"
...
instructions_list.append(
    "Let's give that a try! Press backslash (\\) to do a practice trial."
)
...
for (switch_rate, exemplar) in trial_order:
    ...
    logging.debug(f"Song info: {this_song_info}")
...
################################
# Show final instructions before moving on
instructions_list = ["You're done! Please go to your experimenter."]
